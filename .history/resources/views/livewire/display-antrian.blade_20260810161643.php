<div class="min-h-screen bg-slate-50 text-slate-900">
    <div class="flex min-h-screen flex-col bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.12),_transparent_35%),linear-gradient(180deg,_#f8fafc_0%,_#eef2ff_100%)]">
        ...
        <main class="flex-1 overflow-hidden p-4 xl:p-6">
            <div class="mx-auto flex h-full w-full max-w-[1600px] flex-col gap-5">
                    ...
                    <section class="overflow-hidden rounded-[2rem] border border-blue-100 bg-white shadow-xl shadow-blue-100/60">
                        <div class="grid gap-0 lg:grid-cols-[1.1fr_0.9fr]">
                            <div class="bg-gradient-to-br from-blue-700 to-indigo-800 p-6 text-white">
                                ...
                                <div class="mt-4 text-7xl font-black leading-none xl:text-[3.5rem]">{{ $currentCalling->nomor_antrian }}</div>
                ...
                <section class="flex min-h-0 flex-[1.5] overflow-hidden rounded-[2rem] border border-blue-100 bg-white shadow-xl shadow-blue-100/60">
                    <div class="border-b border-slate-100 px-5 py-4">
                        <div class="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
                            <div>
                                <div class="text-xs font-semibold uppercase tracking-[0.5em] text-blue-600">Ringkasan Per Dinas</div>
                                <h2 class="mt-1 text-2xl font-bold text-slate-600 xl:text-3xl">Antrian Hari Ini</h2>
                            </div>
                            <div class="text-sm text-slate-500">Pantau nomor aktif tiap dinas secara ringkas</div>
                        </div>
                    </div>

                    <div class="h-[calc(100%-76px)] overflow-y-auto px-5 py-5 pb-16 scroll-smooth">
                        <div class="flex flex-col gap-6">
                            @forelse($dinasSummaries as $summary)
                                <article class="flex min-h-[360px] flex-col overflow-hidden rounded-[1.75rem] border border-slate-100 bg-slate-50 shadow-sm">
                                    ...
                                    <div class="border-b border-slate-100 px-5 py-5">
                                        <div class="flex items-start justify-between gap-4">
                                            <div>
                                                ...
                                                <h3 class="mt-2 text-2xl font-bold text-slate-900">{{ $summary->dinas->nama }}</h3>
                                            </div>
                                            <div class="rounded-xl border border-blue-100 bg-white px-4 py-3 text-right shadow-sm">
                                                <div class="text-[10px] uppercase tracking-[0.3em] text-slate-400">Total</div>
                                                <div class="text-2xl font-black text-blue-700">{{ $summary->totalCount }}</div>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="grid flex-1 gap-4 px-5 py-5 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
                                        <div class="rounded-[1.35rem] border border-blue-100 bg-white p-5 shadow-sm">
                                            <div class="text-[11px] uppercase tracking-[0.35em] text-blue-600">Nomor Aktif</div>
                                            <div class="mt-3 text-5xl font-black leading-none text-slate-900 xl:text-6xl">
                                        ...
                                        <div class="grid gap-3">
                                            <div class="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-4 shadow-sm">
                                                <div class="text-xs uppercase tracking-[0.3em] text-blue-600">Menunggu</div>
                                                <div class="mt-2 text-2xl font-black text-blue-700">{{ $summary->waitingCount }}</div>
                                            </div>
                                            <div class="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-4 shadow-sm">
                                                ...
                                                <div class="mt-2 text-2xl font-black text-emerald-700">{{ $summary->callingCount }}</div>
                                            </div>
                                            <div class="rounded-2xl border border-purple-100 bg-purple-50 px-4 py-4 shadow-sm">
                                                <div class="text-xs uppercase tracking-[0.3em] text-purple-600">Selesai</div>
                                                <div class="mt-2 text-2xl font-black text-purple-700">{{ $summary->completedCount }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </article>
                            @empty
                                <div class="rounded-[1.5rem] border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-slate-500">
